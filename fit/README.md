...
- **Snap throat:** the unblended collar construction targets approximately 24.51 mm between rounded noses. Check the working section away from the 2 mm broad-face chamfers. Measure the finished/printed opening; do not treat that construction value as a production tolerance.
...
## Download the current fit coupons

- [Rear seat with aligned helper modifiers](rear-fit-coupon-with-helpers.step) — [body STL](rear-fit-coupon.stl).
- [Front seat with aligned helper modifiers](front-fit-coupon-with-helpers.step) — [body STL](front-fit-coupon.stl).

These are local sections extracted from the finished E10 bracket, at the full 24 mm width. Keep the supplied broad side on the bed and use the same filament, wall count, layers and solid bands as the intended bracket. Import each assembly as one aligned object and change its two helpers to 100% infill modifiers. Cut boundaries change restraint, so a coupon's insertion force is not a calibrated prediction for the complete bracket.

[Rebuild script](build_coupons.py) · [Geometry and mesh checks](coupon-verification.json).
