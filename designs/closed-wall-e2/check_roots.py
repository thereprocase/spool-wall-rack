...
near=(np.linalg.norm(p-[76,0],axis=1)<23)&(ln1>.01)&(ln2>.01)
...
rep={'revision':'E2','root_transition_continuity':'C2 in analytic radial profile; polygonal CAD approximation',
 ...
 'maximum_resolved_vertex_turn_deg_within_23mm_of_rear_seat':maxturn,
...
h=math.sqrt(112.4**2-75**2);n=np.array([75,h])/112.4;rod=np.array([76,0])-.3*n
for ax,rev,col in zip(axes,['closed-wall-e1','closed-wall-e2'],['#89918c','#267f80']):
 ...
 ax.set_xlim(52,100);ax.set_ylim(-25,23)
 ax.set_title('E1 · ABRUPT SHOULDERS' if rev.endswith('e1') else 'E2 · BLENDED ROOTS',loc='left',fontweight='bold',fontsize=14,color='#233e45')
axes[0].annotate('Added circles left steps\nand sharp slot ends',xy=(65,-10),xytext=(54,-22),fontsize=10,color='#fffaf0',arrowprops={'arrowstyle':'->','color':'#fffaf0'})
axes[1].annotate('Continuous thickening\ninto the bearing seat',xy=(64,-5),xytext=(53,-22),fontsize=10,color='#fffaf0',arrowprops={'arrowstyle':'->','color':'#fffaf0'})
fig.suptitle('ACTUAL BRACKET ROOT DETAIL',x=.07,ha='left',fontsize=20,fontweight='bold',color='#233e45')
fig.text(.07,.07,'E2: smooth radial transition • R1.5 concave blends • R0.45 convex rounds • 1.35 mm working finger',fontsize=11,color='#4d656b')
