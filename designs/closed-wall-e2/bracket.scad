// Revision E2: load-aligned snap seats, 2.4 mm perimeter; based on D.
...
// Retainer pockets free both curved fingers; the thick bearing sector joins the arm.
module outline(){
 rounded_profile() union(){
  difference(){legacy_outline();for(x=[rear,front])translate([x,0])circle(r=relief_pocket_r);}
