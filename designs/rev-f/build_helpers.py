...
def main():
    ...
    assembly=cq.Assembly(name='Rev-F-body-and-modifiers')
    ...
    for name,region,z,h in helper_regions(params):
        ...
        if name=='dense-chords-and-seats':
            for label,profile,length in layout.tunnel_saddles(params):
                ...
                # Keep only body-overlapping material plus the existing connected
                # helper; the opening itself is supplied by the printable body.
                helper=helper.fuse(saddle.intersect(mounted)).clean()
