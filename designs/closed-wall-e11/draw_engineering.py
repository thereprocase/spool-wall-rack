"""E11 actual-outline drawing and STL cross-section of the finished chamfers."""
...
ax.annotate('57 mm section depth\nunder inner rod',xy=(90,-45),xytext=(2,-65),arrowprops={'arrowstyle':'->'},fontsize=10)
ax.annotate('Reshaped back arm\nclears 180–220 mm spools',xy=(12,120),xytext=(65,205),arrowprops={'arrowstyle':'->'},fontsize=10)
ax.set(xlim=(-8,240),ylim=(-95,240),aspect='equal',xlabel='Projection from wall (mm)',ylabel='Installed height (mm)',title='Actual E11 profile with nominal dowels and spool envelopes')
...
bx.set_xlim(-6,30);bx.set_ylim(-81,-4);bx.axvline(0,lw=.7,color='#9ba6a0',ls='--');bx.axvline(24,lw=.7,color='#9ba6a0',ls='--')
for a,b in [(0,1.2),(7.6,8.8),(15.2,16.4),(22.8,24)]:
 bx.fill_between([a,b],[-67.5,-67.5],[-15.5,-15.5],color='#b2c9c0',alpha=.5)
bx.annotate('50° to bed plane',xy=(1,-69),xytext=(1,-77),arrowprops={'arrowstyle':'->'},fontsize=10)
bx.annotate('Mirrored 50° bevel',xy=(23,-14),xytext=(-3,-7),arrowprops={'arrowstyle':'->'},fontsize=10)
bx.text(12,-39,'Four continuous\n1.2 mm solid plates',ha='center',fontsize=10)
fig.suptitle('E11 / strong inner seat • 38 mm reinforcement • 50° face chamfers',fontsize=16,fontweight='bold');fig.supxlabel('Actual finished outline and section. Shaded bands locate the slicer-generated plates; the CAD body is a solid envelope.',fontsize=9)
