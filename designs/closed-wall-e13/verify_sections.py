...
def main():
 ...
 import hashlib
 rows=json.loads((D.parent/'closed-wall-e12/section-verification.json').read_text())['cases']
 ...
 for walls in [8,10]:
  row=integrate(reader.GetOutput(),walls);row['revision']='E13';row['summary']=summarize(row)
  ...
  rows.append(row);print('E13',walls,row['summary'],flush=True)
 report={'method':__doc__,'layer_midpoint_mm':LAYER,'requested_added_depth_target_mm':11,'requested_added_depth_maximum_mm':38/3,'measured_finished_added_depth_mm':measured_extra,'source_STL_sha256':hashlib.sha256((D/'body-mounted.stl').read_bytes()).hexdigest(),
  'superseded_E11_I_ratio_gate':MIN_I_RATIO,'superseded_E11_Z_ratio_gate':MIN_Z_RATIO,
  'current_priority':'The requested slightly thicker angular reinforcement uses 11 mm added depth within the existing one-third maximum. This supersedes E11 adjacent-section margin targets.',
  'reference_selection':'Same seat and adjacent reference bands as E11; E10/E11/E12 measurements are retained from their published audits.','cases':rows,
  'limitations':['Local EI and elastic section modulus do not measure notch stress concentration, bearing stress, creep or rupture strength.','Walls are ideal geometric offsets; actual variable-width toolpaths are checked separately.','E13 does not inherit the deeper E11 adjacent-section superiority claim.']}
 ...
 for r in rows:
  for ax,key in zip(axes,['I_z_mm4','min_section_modulus_mm3']):
   if r['revision']=='E11':continue
   ax.plot(XS,np.array(r[key])/1000,label=f"{r['revision']} / {r['walls']} walls",ls={'E10':'--','E12':':','E13':'-'}[r['revision']])
 for ax in axes:
 ...
 fig.suptitle('E13 / angular 11 mm reinforcement: finished section comparison')
