...
def finish(body,p):
 ...
 for top in [False]:
  checkpoint=Path(tempfile.gettempdir())/f'rack-e13-bevel-{cache_key}-{int(top)}.brep'
 ...
 outline=Polygon(p)
 for s in solids[1:]:
  verts,tri=s.copy().tessellate(.005)
  verts=np.array([v.toTuple() for v in verts]);samples=np.vstack([verts,verts[np.array(tri)].mean(1)])
  # Prove the detached material lies in the intended constant-depth bevel,
  # rather than accepting/removing a fragment merely because it is small.
  assert all(depth(v[:2])>1.99999 for v in samples)
  penetration=max(Point(x,y).distance(outline.boundary)-max(0,2-z/RISE_PER_INSET) for x,y,z in samples)
  assert penetration<1e-5,('Remnant enters intended material',penetration)
  remnants.append({'volume_mm3':s.Volume(),'z_min':s.BoundingBox().zmin,'z_max':s.BoundingBox().zmax,'maximum_sampled_penetration_inside_ideal_bevel_mm':penetration})
 print('Bevel remnants',remnants,flush=True)
 assert offcuts<10. and all(r['z_max']<2*RISE_PER_INSET+.01 or r['z_min']>24-2*RISE_PER_INSET-.01 for r in remnants),('Unexpected disconnected material',remnants)
