"""Check E13 raised-rail fit and actual finished chamfers/R2 wall transitions."""
from pathlib import Path
import re,json,math,xml.etree.ElementTree as ET
import numpy as np
import cadquery as cq
from shapely.geometry import Polygon,Point
from continuous_chamfer import RISE_PER_INSET,CHAMFER_ANGLE_DEG
from fastener_geometry import access_profile,FIXINGS,LAND
...
for index,edge in enumerate(spec['edges']):
 ...
 for top in [False,True]:
  # q+z/tan(50deg) <2 must be cut on an intact structural edge.
  for q,z,expected in [(.5,1.,False),(.5,2.,True),(1.5,.75,True)]:
   ...
   expected=bool(poly.contains(Point(*xy)) and distance>max(0,2-z/RISE_PER_INSET))
   # The finished CAD includes asymmetric sloping access roofs. Account for
   # those cuts before checking mirrored exterior bevel material.
   for fixing_y in FIXINGS:
    bore=access_profile(fixing_y,radius=2.6,roof_z=12+2.6*2**.5,round_top=0)
    if (xy[0]>=LAND and access_profile(fixing_y).covers(Point(xy[1],Z))) or (xy[0]<=LAND+.01 and bore.covers(Point(xy[1],Z))):expected=False
   if abs(distance-max(0,2-z/RISE_PER_INSET))<.02:continue
...
# Independently measure finished straight underside bevels at X=160.
# Search actual STEP material boundaries, rather than returning construction inputs.
angle_checks=[]
for top in [False,True]:
 zs=np.array([.25,.75,1.25,1.75,2.25]);ys=[]
 for z in zs:
  lo,hi=-33.,-28.
  for _ in range(32):
   mid=(lo+hi)/2
   if contains((160,mid,24-z if top else z)):hi=mid
   else:lo=mid
  ys.append((lo+hi)/2)
 slope=np.polyfit(ys,zs,1)[0];angle=math.degrees(math.atan(abs(slope)))
 assert abs(angle-50)<.03,(top,angle)
 angle_checks.append({'top_face':top,'z_from_face_mm':zs.tolist(),'measured_boundary_y_mm':ys,'measured_angle_above_face_deg':angle})
assert np.max(abs(np.array(angle_checks[0]['measured_boundary_y_mm'])-angle_checks[1]['measured_boundary_y_mm']))<.002
angular=[]
for label,a,b in [('wall_side_taper',(35,-32),(76,-43)),('flat_bottom',(76,-43),(104,-43)),('outer_side_taper',(104,-43),(145,-32))]:
 samples=[]
 for x in np.linspace(a[0]+3,b[0]-3,9):
  expected_y=a[1]+(b[1]-a[1])*(x-a[0])/(b[0]-a[0]);lo,hi=expected_y-.1,expected_y+.1
  for _ in range(28):
   mid=(lo+hi)/2
   if contains((x,mid,12)):hi=mid
   else:lo=mid
  measured=(lo+hi)/2;assert abs(measured-expected_y)<.01,(label,x,expected_y,measured)
  samples.append({'x_mm':float(x),'expected_y_mm':float(expected_y),'measured_y_mm':measured})
 angular.append({'feature':label,'nominal_endpoints_mm':[a,b],'angle_above_horizontal_deg':math.degrees(math.atan2(b[1]-a[1],b[0]-a[0])),'actual_STEP_boundary_samples':samples})
report={'revision':'E13','front_lift_mm':12,'rear_center_mm':[90,0],'front_center_mm':[190,12],'profile_area_mm2':poly.area,'minimum_flange_clearance':worst,'spool_sweep_cases':len(rows),'chamfer_mm':2,'mirrored_chamfer_checks':checks,'body_R2_arc_checks':fillets,'structural_junctions_full_chamfer':junctions,'finger_material_checks':finger_checks,'washer_landings':land,'limitations':['Nominal seated 2D flange sweep; not insertion or loaded/deformed contact validation.','Thin snap noses retain their separately dimensioned radii, not a blanket R2.','The wall contact strip ends retain square contact corners; exposed body-facet transitions are R2.','E13 numerical results are reported separately; physical qualification is unperformed.']}
report['chamfer_face_inset_mm']=2
report['chamfer_rise_mm']=2*RISE_PER_INSET
report['measured_chamfer_angles']=angle_checks
report['layer_run_at_0p2_mm']=.2/RISE_PER_INSET
report['angular_underside_checks']=angular
report['limitations'][-1]='E13 section, reduced-model and full 3D numerical results are reported separately from these geometry checks; physical qualification is unperformed.'
(D/'engineering-verification.json').write_text(json.dumps(report,indent=2));print('Clearance, hardware, finger preservation, mirrored chamfers and measured 50 degree angles pass',flush=True)
