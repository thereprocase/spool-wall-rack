...
def finish(body,p):
 ...
 for i,(a,c) in enumerate(zip(p,np.roll(p,-1,axis=0))):
  ...
  wire=cq.Wire.makePolygon([V(-ea,outside+da),V(L+ec,outside+dc),V(L+ec,-outside),V(-ea,-outside)],close=True)
  cutters.append(cq.Solid.extrudeLinear(wire,[],cq.Vector(*(2*outside*n),-2*outside)))
  spec.append({'a':a.tolist(),'b':c.tolist(),'depth_start_mm':float(da),'depth_end_mm':float(dc)})

 cache_key=hashlib.sha256(json.dumps(spec,sort_keys=True).encode()).hexdigest()[:16]
 for top in [False]:
  checkpoint=Path(tempfile.gettempdir())/f'rack-e10-bevel-{cache_key}-{int(top)}.brep'
  ...
  for i in range(0,len(tools),24):
   body=body.cut(*tools[i:i+24])
   if not body.isValid():body=body.fix()
   if not body.isValid():
    cq.exporters.export(body,str(checkpoint.with_suffix('.failed.brep')))
 ...
 assert offcuts<2. and all(r['z_max']<2.01 or r['z_min']>21.99 for r in remnants),('Unexpected disconnected material',remnants)
 ...
 offcuts*=2
 return body,{'revision':'E10','chamfer_mm':2,'width_mm':24,'order':['R2 side-profile fillets','extrude','continuous face chamfers','fastener bores'],'mirroring':'Finished Z=0..12 half mirrored about Z=12 and fused before fastener cuts','runouts':'Only thin dowel-retainer regions; no structural edge-end fades','detached_bevel_remnants_removed_mm3':offcuts,'bevel_remnant_audit':remnants,'edges':spec}
