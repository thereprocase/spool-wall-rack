...
def polys(f):
 ...
 for e in root.findall('{http://www.w3.org/2000/svg}path'):
  for s in e.attrib['d'].split('M')[1:]:out.append(np.array([list(map(float,x)) for x in re.findall(r'([-+\d.eE]+),([-+\d.eE]+)',s)])*[1,-1])
 return out
...
angle=math.degrees(math.acos(50/112.4));rotation=angle-90
ret=[rotate(p,rotation)+[90,0] for p in rp]
checks=[]
for diameter in np.arange(180,220.001,.1):
 R=diameter/2;h=math.sqrt((R+12.4)**2-50**2);c=np.array([140,h])
 checks.append((diameter,gap(ps,c,R),gap(ret,c,R),math.degrees(math.atan2(h,50))))
vals=np.array(checks);vv=np.concatenate(ps)
throat=2*((13+1.35/2)*math.cos(math.radians(19))-1.35/2)
report={'revision':'E7','status':'geometry prototype; no slicing, snap-force, fatigue, or strength validation',
 ...
 'broad_face_chamfer_mm':3,'main_body_corner_fillet_mm':2,
 'root_transition':'C2 quintic radial blend; 25 to 75 degrees from bearing axis',
 'concave_root_fillet_mm':2,'convex_edge_round_mm':.45,'wrap_deg':218,'seat_diameter_mm':26,'finger_bending_thickness_mm':1.35,'bearing_radial_thickness_mm':6,
...
h=math.sqrt(112.4**2-50**2);C=np.array([140,h])
...
ax.set_xlim(-5,251);ax.set_ylim(-60,215)
ax.set_title('MIRRORED, LOAD-ALIGNED SEATS',loc='left',fontweight='bold',fontsize=13)
ax.text(0,-53,'200 mm reference spool • 100 mm seat centers',fontsize=10,color='#4d656b')
ax=axes[1];ax.add_patch(PathPatch(path(ret),fc='#267f80',ec='#164a53',lw=1))
n=(C-[90,0])/112.4;rod=np.array([90,0])-.3*n
ax.add_patch(Circle(rod,12.7,fc='#d3b27a',ec='#977542'))
contact=rod+12.7*n;bearing=rod-12.7*n
...
for theta,label,pos in [(161,'1.35 mm flexible finger',(40,19)),(379,'Tangent rounded nose',(97,-14))]:
 p=rotate(np.array([[13.675*math.cos(math.radians(theta)),13.675*math.sin(math.radians(theta))]]),rotation)[0]+[90,0]
 ax.annotate(label,xy=p,xytext=pos,arrowprops={'arrowstyle':'->','color':'#164a53'},fontsize=10)
ax.annotate('',xy=np.array([90,0])+20*n,xytext=np.array([90,0])+29*n,arrowprops={'arrowstyle':'->','lw':2,'color':'#164a53'})
ax.set_xlim(38,123);ax.set_ylim(-34,34)
ax.set_title('REAR SEAT DETAIL',loc='left',fontweight='bold',fontsize=13)
fig.suptitle('E7 • SHORTER SPAN / FULL-HEIGHT WALL CONTACT',x=.055,ha='left',fontsize=21,fontweight='bold',color='#233e45')
fig.text(.055,.025,'2.4 mm structural perimeter • 1.2 mm continuous plates • prototype geometry; insertion force and bridge cells modeled; sliced paths and insertion unverified',fontsize=10,color='#4d656b')
