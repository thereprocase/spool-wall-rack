...
def read_stl(name,printed=False):
 ...
 if printed:v[:,:,0]=250-v[:,:,0];v[:,:,1]=182-v[:,:,1]
...
def render(name,parts,title,subtitle,view=(19,-59),lim=None,dims=None):
 ...
 if lim is not None:
  focus=np.asarray(lim)@np.array([right,up,eye]).T
  lo=focus[:,:2].min(0);hi=focus[:,:2].max(0)
 ...
 draw.rectangle((0,0,W,160),fill=bg);draw.rectangle((0,H-95,W,H),fill=bg)
 fonts=str(Path(matplotlib.get_data_path())/'fonts'/'ttf')+'/'
 ...
 draw.text((90,H-70),'REV G · ACTUAL CAD · STIFFNESS / WEIGHT STUDY',font=ImageFont.truetype(fonts+'DejaVuSans.ttf',17),fill='#667779')
 im.save(D/name)

if __name__=='__main__':
 body=read_stl('body-mounted.stl')
 render('progress-exterior.png',[(body,(.23,.52,.54))],
  'REV G / MATERIAL WHERE IT WORKS',
  'E13 outline and interfaces · angular windows · selective reinforcement in development',view=(17,-57))
