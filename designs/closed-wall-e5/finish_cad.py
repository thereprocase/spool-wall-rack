"""Finish E5 broad-face edges and preserve solid material behind the chamfers.
...
if __name__=='__main__':
 ...
 for a,b in zip(p,np.roll(p,-1,axis=0)):
  ...
  if L<14:continue
  da=min(np.linalg.norm(a-[x,0]) for x in [76,226]);db=min(np.linalg.norm(b-[x,0]) for x in [76,226])
 ...
 sleeves=[cq.Solid.makeCylinder(5.15,75,cq.Vector(-1,y,12),cq.Vector(1,0,0)) for y in [138,12]]
 cavities=[]
 ...
 for y in [138,12]:result=result.cut(cq.Solid.makeCylinder(2.75,75,cq.Vector(-1,y,12),cq.Vector(1,0,0)))
 ...
 printed=result.rotate((0,0,0),(0,0,1),180).translate((250,156,0))
 ...
 record={'revision':'E5','chamfer_mm':CHAMFER,'body_corner_fillet_mm':2,'width_mm':WIDTH,'face_and_internal_plate_mm':FACE,
