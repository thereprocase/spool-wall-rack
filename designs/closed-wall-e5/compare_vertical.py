...
for rev in ['E4','E5']:
 ...
 for dia in [180,200,220]:
  h=math.sqrt((dia/2+12.4)**2-75**2);bottom=h-dia/2;top=h+dia/2
 ...
 out['revisions'][rev]={'bracket_height_mm':float(np.ptp(p[:,1])),'back_contact_extent_y_mm':[ymin,ymax],'back_contact_area_mm2':sum(f.Area() for f in wall),'upper_screw_to_lowest_back_contact_mm':138-ymin,'sections':[section(shape,x) for x in [125,175]],'spool_envelopes':spool}
 print(rev,'checked',flush=True)
a=out['revisions']['E4'];b=out['revisions']['E5']
out['E5_to_E4_local_EI_ratios']={str(s['x_from_wall_mm']):t['I_about_width_axis_mm4']/s['I_about_width_axis_mm4'] for s,t in zip(a['sections'],b['sections'])}
out['E5_to_E4_idealized_upper_screw_tension_ratio']=a['upper_screw_to_lowest_back_contact_mm']/b['upper_screw_to_lowest_back_contact_mm']
out['assumptions']=['Perfectly flat wall. Geometric contact area is not uniformly loaded bearing area.','Screw-tension ratio uses a point compression reaction at the lowest wall contact and all overturning tension at the upper screw; actual contact and bolt forces require analysis.','Section properties use 0.2 mm slices of the final CAD with all modeled solid filled and all air volumes empty. Infill gets zero strength credit.','Local EI ratio is not whole-bracket stiffness; connection compliance, shear, anisotropy, buckling and creep are excluded.']
...
h=math.sqrt(112.4**2-75**2);bottom=h-100
for ax,rev in zip(axes,['E4','E5']):
 folder=D.parent/('closed-wall-'+rev.lower());p=profile(folder);lo=p[:,1].min();hi=p[:,1].max()
 ax.set_aspect('equal');ax.axis('off');ax.set_xlim(-50,294);ax.set_ylim(-109,220)
 ax.add_patch(Rectangle((-12,-99),12,310,fc='#dedfd9',ec='#a0aaa9',hatch='///',lw=.5))
 ax.add_patch(Polygon(p,fc='#267f80',ec='#164a53',lw=.7))
 ax.add_patch(Circle((151,h),100,fc='#e7d8b9',ec='#a38f69',alpha=.55,lw=1))
 for x in [76,226]:
  n=(np.array([151,h])-[x,0])/112.4;c=np.array([x,0])-.3*n
  ax.add_patch(Circle(c,12.7,fc='#d3b27a',ec='#977542',lw=1))
 for y,length in [(138,20),(12,60)]:
 ...
 ax.text(275,(lo+bottom)/2,f'{bottom-lo:.1f} mm\nbelow spool',ha='left',va='center',fontsize=10,color='#a95737')
 ax.text(151,h+8,'Ø200 mm spool',ha='center',color='#665941',fontsize=13)
 ax.text(151,h-10,'Nominal seated position',ha='center',color='#665941',fontsize=9)
 ax.text(-8,211,'WALL',fontsize=9,color='#667779')
 ax.set_title(rev+' / '+('PREVIOUS DEEP ARM' if rev=='E4' else 'COMPACT CANDIDATE'),loc='left',fontweight='bold',fontsize=14,color='#233e45')
 ax.text(8,-100,('Clipped toe; back contact ends above arm underside' if rev=='E4' else 'Flat back reaches the arm underside'),fontsize=10,color='#4d656b')
fig.suptitle('45 mm less height below the spool',x=.06,ha='left',fontsize=23,fontweight='bold',color='#233e45')
fig.text(.06,.025,'Actual CAD profiles • identical rails, spool and scale • shallower arm has lower local bending stiffness',fontsize=11,color='#4d656b')
