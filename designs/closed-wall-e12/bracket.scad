// Revision E12: solid exterior profile with load-aligned snaps.
...
front_lift=12;
// Shallow C2 underside blend: 9.5 mm target; 12.6667 mm absolute cap.
// E12 prioritizes the requested compact outline over E11 adjacent-section margins.
// Functional seat centers and the spool-facing outline stay fixed.
seat_extra_depth=9.5;
seat_spread=24;
...
module arm_outline(){
 difference(){
  offset(r=2) offset(r=-2) offset(r=-2) offset(r=2) union(){
   ...
   // E12 removes the obsolete projecting lower screw-head pad.
...
if(part=="bracket")assert(false,"Build the finished E12 solid with build_step.py");
