...
meshes={rev:load_mesh(D.parent/f'closed-wall-{rev}'/'body-mounted.stl') for rev in ['e10','e12']}
rows=[]
for label,c,rot in zip(['inner','outer'],[[90,0],[190,12]],rots):
 for side,a0,a1 in [('left',150,195),('right',345,390)]:
  ...
  for z in [.1,.7,1.7,3.,12.,21.,22.3,23.3,23.9]:
   ...
   q=layer_polygons(meshes['e12'],z).intersection(region)
   ...
   rows.append({'seat':label,'finger':side,'print_z_mm':z,'reference_area_mm2':a.area,'E12_area_mm2':q.area,'missing_beyond_0p025mm_mm2':missing,'added_beyond_0p025mm_mm2':added})
...
for label,bounds in [('inner',[56,124,-30,24,-1,25]),('outer',[159,213,-22,38,-1,25])]:
 ...
 for face,view in [('front',(25,-57)),('reverse',(25,57))]:
  render(f'finish-{label}-{face}.png',[(v,(.23,.52,.54))],f'E12 / {label.upper()} RETAINER / {face.upper()} FACE','Actual finished CAD crop · preserved finger profile · smooth bevel runouts',view=view,lim=focus)
