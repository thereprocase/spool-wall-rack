"""Cross-check the E12 handoff and its published result fields.
...
CAD=ROOT/'designs/closed-wall-e12';MANIFEST=D/'release-verification.json'
...
def main():
 ...
 assert len(fields)==11 and sum(f['dimension']==3 for f in fields)==6
 ...
 assert sec['requested_added_depth_target_mm']==9.5
 assert sec['measured_finished_added_depth_mm']<=38/3+.02
 close(build['rear_underside_added_depth_mm'],9.5)
 for case in sec['cases']:
  if case['revision']!='E12':continue
 ...
 files=[ROOT/n for n in ['.gitignore','.gitattributes','AGENTS.md','README.md','DESIGN.md','DESIGN-JOURNAL.md','E10-ENGINEERING-GUIDE.md','E11-ENGINEERING-GUIDE.md']]
 ...
 files += [ROOT/'designs/closed-wall-e10/body-mounted.stl',ROOT/'designs/closed-wall-e11/body-mounted.stl',ROOT/'designs/closed-wall-e11/profile.svg',ROOT/'designs/closed-wall-e11/section-verification.json',ROOT/'designs/closed-wall-e6/material-reference-data.json']
 for rev in ['e10','e11']:
  for n in [8,10]:
   for h in [1,2]:files.append(D.parent/'e11'/f'{rev}-{n}w-h{h}-full-results.json')
  files += [D.parent/'e11'/f'{rev}-8w-h1-full-solution.npz',D.parent/'e11'/f'{rev}-8w-h1-outer_only-results.json']
 ...
 for path in sorted(files):
  ...
  if path.suffix=='.md':
   ...
   for url in re.findall(r'\]\(([^)]+)\)',text):
    ...
    links+=1
 report={'revision':'E12','status':'Published-artifact and numerical consistency checks pass; physical qualification is unperformed','fields_checked':fields,'relative_markdown_links_checked':links,'artifact_hash_method':'SHA256; LF-normalize text as marked, preserve exact CAD/binary bytes','artifacts':artifacts}
