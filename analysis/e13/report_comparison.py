"""Matched E10/E12/E13 comparison from the actual solved reduced fields."""
...
def folder(rev):return D if rev=='e13' else D.parent/('e12' if rev=='e12' else 'e11')
...
for n in [8,10]:
 ...
 old,previous,slim=local
 deltas.append({'walls':n,'E13_stress_reduction_vs_E10_percent':100*(1-slim['near_seat_p99_MPa']/old['near_seat_p99_MPa']),'E13_movement_reduction_vs_E10_percent':100*(1-slim['front_mm_at_E1000']/old['front_mm_at_E1000']),'E13_stress_change_vs_E12_percent':100*(slim['near_seat_p99_MPa']/previous['near_seat_p99_MPa']-1),'E13_movement_change_vs_E12_percent':100*(slim['front_mm_at_E1000']/previous['front_mm_at_E1000']-1)})
outer=[]
for rev in ['e10','e12','e13']:
...
for ax,rev in zip(axes,['e10','e12','e13']):
 ...
 ax.set(xlim=(55,127),ylim=(-46,22),aspect='equal',xlabel='Projection X (mm)',title=f"{rev.upper()} / added depth {dict(e10=0,e12=9.5,e13=11)[rev]:g} mm\nNear-seat p99 {r['near_seat_p99_MPa']:.2f} MPa")
 ax.set_ylabel('Height Y (mm)')
fig.colorbar(art,ax=axes,shrink=.7,label='Element von Mises stress (MPa)',extend='max')
fig.suptitle('E13 / angular reinforcement / matched 8-wall solved fields',fontsize=16,fontweight='bold')
...
text='''# E13 matched comparison with E10 and E12
...
for r in deltas:text+=f"| {r['walls']} | {r['E13_stress_reduction_vs_E10_percent']:.2f}% | {r['E13_movement_reduction_vs_E10_percent']:.2f}% | {r['E13_stress_change_vs_E12_percent']:.2f}% | {r['E13_movement_change_vs_E12_percent']:.2f}% |\n"
...
text+=f"\nE13 reduces this outer-only near-seat statistic by **{summary['E13_outer_only_stress_reduction_vs_E10_percent']:.2f}%** relative to E10.\n"
text+='''
