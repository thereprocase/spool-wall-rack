"""Render actual solved E13 tetrahedral fields, including real cut sections."""
...
def main():
 grids={n:grid(f'print-material-{n}w-h1') for n in [8,10]}
 for n,(g,r) in grids.items():
  ...
  fig.suptitle(f'E13 / solved 3D stress / {n} walls / 12 kg equivalent bracket load',fontsize=17,fontweight='bold')
 ...
 for col,(n,(g,r)) in enumerate(grids.items()):
  for row,(origin,normal,coords) in enumerate([((0,0,8.2),(0,0,1),(0,1)),((90,0,0),(1,0,0),(2,1))]):
   ...
   else:ax.set(xlim=(-1,25),ylim=(-45,-10),xlabel='Bracket width / print Z (mm)',ylabel='Height Y (mm)',title=f'{n} walls / transverse cut\nX = 90 mm')
 fig.colorbar(pc,ax=axes,shrink=.65,pad=.04,label='Element von Mises stress (MPa)',extend='max')
 fig.suptitle('E13 / sections through the solved 3D stress field',fontsize=17,fontweight='bold')
 ...
 fig.suptitle('E13 / upper screw landing / 500 N washer compression',fontsize=16,fontweight='bold')
 ...
 print('E13 full, cutaway, transverse/plate and landing stress views rendered',flush=True)
