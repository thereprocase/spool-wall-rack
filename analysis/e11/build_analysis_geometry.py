"""E11 analysis-only ideal walls/plates, cut from the actual finished STEP.
...
SRC=D.parents[1]/'designs/closed-wall-e11'
...
def build(n):
 ...
 key=hashlib.sha256(json.dumps([p.tolist(),wall,spec],sort_keys=True).encode()).hexdigest()[:16]
 ...
 if cache.exists():air=cq.importers.importBrep(str(cache)).val()
  ...
  air=b.prism(np.array(inner.exterior.coords)[:-1],1.2,6.4)
 ...
 cores=[air,b.prism(np.array(inner.exterior.coords)[:-1],8.8,6.4),air.mirror('XY').translate((0,0,24))]
 ...
 return {'walls':n,'effective_wall_mm':wall,'volume_mm3':body.Volume(),'valid':body.isValid(),'solid_count':len(body.Solids()),'source_STEP_sha256':hashlib.sha256((SRC/'body-only.step').read_bytes()).hexdigest(),'core_model':'Finished chamfer planes offset inward in XY; ideal tunnel surrounds; four 1.2 mm continuous plates; zero sparse-infill credit'}
