"""Render actual solved E11 tetrahedral fields, including real cut sections."""
...
def main():
 ...
 for n,(g,r) in grids.items():
  ...
  fig.suptitle(f'E11 / solved 3D stress / {n} walls / 12 kg equivalent bracket load',fontsize=17,fontweight='bold')
 ...
 for col,(n,(g,r)) in enumerate(grids.items()):
  for row,(origin,normal,coords) in enumerate([((0,0,8.2),(0,0,1),(0,1)),((90,0,0),(1,0,0),(2,1))]):
   ...
   if row==0:
    ax.set(xlim=(-3,211),ylim=(-74,181),xlabel='Projection X (mm)',ylabel='Height Y (mm)',title=f'{n} walls / in-plane section\nInternal plate at Z = 8.2 mm')
    ax.axvline(90,color='#333333',lw=.7,ls='--')
   else:ax.set(xlim=(-1,25),ylim=(-73,-8),xlabel='Bracket width / print Z (mm)',ylabel='Height Y (mm)',title=f'{n} walls / transverse cut\nX = 90 mm')
 fig.colorbar(pc,ax=axes,shrink=.65,pad=.04,label='Element von Mises stress (MPa)',extend='max')
 fig.suptitle('E11 / sections through the solved 3D stress field',fontsize=17,fontweight='bold')
 ...
 fig.suptitle('E11 / upper screw landing / 500 N washer compression',fontsize=16,fontweight='bold')
 ...
 print('E11 full, cutaway, transverse/plate and landing stress views rendered',flush=True)
