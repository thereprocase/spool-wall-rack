...
solver=PyPardisoSolver();up=exact.copy();up[free]=solver.solve(K[free][:,free].tocsr(),-K[free][:,fixed]@up[fixed]);solver.free_memory(everything=True)
out['PARDISO_affine_displacement_error_mm']=float(abs(up-exact).max())
out['PARDISO_vs_SuperLU_displacement_difference_mm']=float(abs(up-u).max())
assert out['PARDISO_affine_displacement_error_mm']<1e-10
assert out['patch_stress_error_MPa']<1e-8
assert abs(out['FE_energy_Nmm']-out['analytic_energy_Nmm'])<1e-8
Path(__file__).with_name('solver-verification3d.json').write_text(json.dumps(out,indent=2))
print(out)
