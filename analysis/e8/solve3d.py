"""Reproducible small-strain isotropic 3D FEM, constant-strain tetrahedra.
...
if name.startswith('landing'):
 ...
 for x,sgn,label in [(90,-1,'rear_seat'),(190,1,'front_seat')]:
  r=np.hypot(cent[:,0]-x,cent[:,1]);theta=np.arctan2(cent[:,1],cent[:,0]-x);target=np.arctan2(-58.86,sgn*H)
  da=np.arctan2(np.sin(theta-target),np.cos(theta-target))
  mask=(abs(r-13)<.12)&(abs(da)<np.deg2rad(25))
  patch(mask,[sgn*H,-58.86,0.],label)
 for y in [164,40]:
