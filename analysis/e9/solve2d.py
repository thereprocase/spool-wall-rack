"""Plane-stress FEM of projected E9 outline; integrated ideal print thickness.
...
D=Path(__file__).resolve().parent;SRC=D.parents[1]/'designs/closed-wall-e9';sys.path.insert(0,str(SRC))
...
for x,s,label in [(90,-1,'rear_seat'),(190,1,'front_seat')]:
 ...
 theta=np.arctan2(ec[:,1]-yy,ec[:,0]-x);target=np.arctan2(-V,s*H);da=np.arctan2(np.sin(theta-target),np.cos(theta-target));mask=(abs(np.hypot(ec[:,0]-x,ec[:,1]-yy)-13)<.12)&(abs(da)<np.deg2rad(25));es=edges[mask];wt=length[mask];wt=wt/wt.sum()/2
 for j in range(2):
  for k,F in enumerate([s*H,-V]):np.add.at(f,2*es[:,j]+k,wt*F)
