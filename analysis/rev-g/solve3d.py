"""Rev G small-strain isotropic 3D FEM, constant-strain tetrahedra.
...
source_mesh=sys.argv[1];name=('outer-only-' if '--outer-only' in sys.argv else '')+source_mesh
d=np.load(D/f'{source_mesh}.npz');p=d['p'];t=d['t'];N=len(p)
...
if name.startswith('landing'):
 ...
 for x,sgn,label in [(90,-1,'rear_seat'),(190,1,'front_seat')]:
  ...
  force=[sgn*H,-V,0.]
  if '--outer-only' in sys.argv and label=='rear_seat':force=[0.,0.,0.]
  patch(mask,force,label)
 for y in [164,40]:
...
res['source_mesh']=source_mesh
res['load_case']='outer_only' if '--outer-only' in sys.argv else 'full'
res['load_compliance_Nmm_at_E1000']=float(f@u)
