"""Rev G ideal perimeter/skin/rib domain from finished CAD; no sparse infill credit."""
from pathlib import Path
import importlib.util,sys,json,hashlib
import cadquery as cq
from shapely import union_all
D=Path(__file__).resolve().parent;ROOT=D.parents[1];SRC=ROOT/'designs/rev-g'
...
def main():
    ...
    body_dir=Path(sys.argv[sys.argv.index('--body-dir')+1]) if '--body-dir' in sys.argv else SRC
    ...
    body=cq.importers.importStep(str(body_dir/'body-only.step')).val().translate((-250,-182,0)).rotate((0,0,0),(0,0,1),180)
    ...
    row={'revision':'G','parameters':params,'layout_sha256':layout.layout_hash(params),
         'body_STEP_sha256':hashlib.sha256((body_dir/'body-only.step').read_bytes()).hexdigest(),
         ...
         'model':f"Finished E13 perimeter/chamfer/tunnel guards intersected with Rev G body; new window collars and full-width dense regions; two {params['planes']} internal {plane:g} mm planes and {skin:g} mm broad skins; zero sparse-infill credit."}
