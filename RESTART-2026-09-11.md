# Restart handoff: G material correction and GPU transition
...
- [GPU software review](analysis/rev-g2/GPU-FEM-REVIEW.md) replaces the prolonged
  CPU tetrahedral meshing investigation as the next direction. Discovery
  Explore is commercial with a trial, not an unrestricted free application.
  DTU's source is publicly downloadable; exact reuse license terms and the
  upstream build must still be checked. No GPU replacement is installed or
  benchmarked. The cost-free route remains to be selected and demonstrated.
...
2. Select a free GPU route with verified license/build. Reproduce an upstream
   benchmark on the actual RTX 3080 Ti before attempting the bracket. Measure
   runtime and peak VRAM; do not inherit A100/4090 paper timings.
3. Prove thin-material representation, zero bridge/void credit, raw stress
   export and nonzero-gap contact on small fixtures. Avoid returning to
   unbounded whole-part tetrahedral meshing.
4. Recheck G with corrected 3D material and mesh sensitivity, then the changed
   spool load. Repair the unfinished reduced-matrix application as needed.
